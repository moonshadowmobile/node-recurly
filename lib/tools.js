...
module.exports = function(config){
				...
				try{
					if(res.statusCode >= 400){
						...
						if(error_states.indexOf(res.statusCode) > -1){
							tools.parseErrorXML(responseData, function(error, result){
								if(error) {
									return callback(error);
								}

								var error = new Error(tools.getErrorMessage(result));
								error.type = res.statusCode;
								callback(error);
							});
						}else{
	...
	this.parseErrorXML = function(xml, callback) {
		var options = {
			'explicitRoot': false,
			'ignoreAttrs': false,
			'emptyTag': null,
		...
		this._parseXML(xml, options, callback);
	};
	...
	this._parseXML = function(xml, options, callback){
		console.log(xml);
	...
	this.getErrorMessage = function(result) {
		var msg = [];

		if(result.error){
			if(Array.isArray(result.error)) {
				result.error.forEach(function(error) {
					msg.push(tools.parseErrorObj(error));
				});				
			} else {
				msg.push(tools.parseErrorObj(result.error));
			}
		}
		if(result.description && result.description['#']){
			msg.push(result.description['#']);
		}
		if(result.details){
			msg.push(result.details);
		}

		return msg.join(', ');
	};

	this.parseErrorObj = function(error) {
		var msg = [];

		if(error["@"]) {
			msg.push((error["@"].field + " "));
		}
