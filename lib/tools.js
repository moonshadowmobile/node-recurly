					...
					if(res.statusCode >= 400){
						...
						if(error_states.indexOf(res.statusCode) > -1){
							tools.parseXML(responseData, function(error, result){
								...
								var msg = [];
								if(result.errors.error && result.errors.error["@"]){
									msg.push((result.errors.error["@"].field + " ") + result.errors.error["#"]);
								}
						...
						if(responseData != ""){
							tools.parseXML(responseData, function(error, result){
								...
								// Shed all of the XMLisms
								// @TODO add option to disabled transcribe
